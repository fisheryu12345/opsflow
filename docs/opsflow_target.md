...
**定位：** 平台对外的统一 API 出口，独立于 opsflow 核心。当前 `opsflow/core/apigw/` 的功能将迁移到此 app。

**当前实现：**

| 端点 | 方法 | 说明 | 状态 |
|------|------|------|------|
| `POST /api/open/v1/executions/` | 触发执行 | 提供 template_id 或 pipeline_tree | ✅ 完成 |
| `GET /api/open/v1/executions/{id}/` | 查询执行状态 | 返回状态/node_status/结果 | ✅ 完成 |
| `GET /api/open/v1/templates/` | 列出模板 | 分页返回可用模板 | ✅ 完成 |
...
| `apigw/` (待迁移) | 外部 API (功能将迁移到 `open_api` app) | ⏳ 迁移中 |
...
- [ ] **Open API 独立化** — 将 `opsflow/core/apigw/` 功能迁移到 `backend/open_api/`，使其成为跨子产品的统一对外出口
