...
| 查看这个应用的依赖拓扑 | 打开拓扑图、浏览关系链 | TopologyCanvas (G6) | TopologyService | Neo4j 关系 |
...
| 把应用进程注册到 CMDB | 部署 ProcessManager，自动发现进程 | 进程列表 | ProcessManager | Neo4j Process 节点 |
| 查看进程间的跨主机调用关系 | 打开 DR 拓扑图，查看 CALLS 关系链 | DR拓扑Tab (G6) | TopologyService / AssociationService | Neo4j CALLS 关系 |
| 配置业务应用的灾备切换 | 选择 DrGroup，AI 生成 DR pipeline | CreateTemplateWizard → DR选项 | LLMService + TemplateDRMixin | DrSite / DrGroup / FAILOVER_TO |
...
| **CMDB** | `views/apps/cmdb/` | TopologyCanvas / DynamicTable / ResourceDetail / DR拓扑Tab | G6 (拓扑树+力导向) |
...
| `ws/cmdb/topology/` | CMDB 拓扑实时协作 + DR 切换进度推送 | CMDB | 规划（DR 设计中） |
...
| 变更追踪 | 资源变更历史记录 | 📅 规划 |
| 自动发现 | ProcessManager 进程定时采集 + HTTP 上报 CMDB API | 🔄 DR Phase 1 设计中 |
...
| `process` | 3 | process_start, process_stop, process_status | medium~high |
| `servicenow` | 5 | create_incident, create_change_request, get_cmdb_ci | low |
| `ai` | 1 | ai_text_gen | low |
...
- [ ] **Process CMDB 建模** — Process 模型定义 + AssociationType (RUNS_ON/CALLS/EXPOSES) + 种子数据
- [ ] **ProcessManager 实现** — 主机端定时采集 (ss+ps) + CMDB API 上报 + CALLS 关系自动发现
- [ ] **进程生命周期 OpsFlow 原子** — process_start / process_stop / process_status（继承 TowerBasePlugin）
...
- [ ] **DR 站点拓扑可视化** — DrSite + DrGroup 模型 + SITE_CONTAINS/FAILOVER_TO 关系 + G6 力导向图
- [ ] **AI 辅助 DR 编排** — CreateTemplateWizard Step 2 新增 DR 切换选项 + 基于 Neo4j 拓扑生成 DR pipeline
