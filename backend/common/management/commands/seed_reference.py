...
class Command(BaseCommand):
    ...
    def _seed_connector_definitions(self):
        ...
        c_count = 0
        for c in connectors:
            _, created = ConnectorDefinition.objects.get_or_create(code=c["code"], defaults=c)
            if created:
                c_count += 1
        self.stdout.write(f">>> Connector Definitions: {c_count} new, {len(connectors) - c_count} existing")
