...
def send_email_task(subject, body, receiver_email,
    ...
    if sender_pass is None:
        sender_pass = os.getenv('EMAIL_PASSWORD', 'rnfjkkwocuuicbch')
...
@shared_task(retry_delay=180, max_retries=1)
def send_report_email_task(subject, body, receiver_email, pdf_attachment=None,
                            smtp_server='smtp.qq.com', smtp_port=587,
                            sender_email=None, sender_pass=None):
    """
    发送带PDF附件的绩效报告邮件
    ...
    Args:
        ...
        body: HTML邮件正文
        receiver_email: 收件人邮箱
        pdf_attachment: PDF附件字节数据 (bytes)，None则不发送附件
        smtp_server: SMTP服务器地址
        smtp_port: SMTP服务器端口
        sender_email: 发件人邮箱
        sender_pass: 发件人授权码
    ...
    if pdf_attachment is not None:
        ...
        part.add_header('Content-Disposition', 'attachment', filename='performance_report.pdf')
        msg.attach(part)
    ...
    try:
        ...
        server.login(sender_email, sender_pass)
        server.sendmail(sender_email, [receiver_email], msg.as_string())
        logger.info('报告邮件发送成功 -> %s', receiver_email)
        return True
    except smtplib.SMTPAuthenticationError:
        logger.error('邮件发送失败: 认证错误')
        return False
    except smtplib.SMTPConnectError:
        logger.error('邮件发送失败: 无法连接到SMTP服务器')
        return False
