...
import math
FSM = 'sync_kline_data'
FSM_CONTRACT = 'sync_contract_list'
...
def sync_contract_list_from_tqsdk(api=None):
    ...
    try:
        ...
        log_error(FSM_CONTRACT, f"同步合约列表失败: {e}", notify=True)
        traceback.print_exc()
        return {
            'synced': 0,
            'updated': 0,
    ...
    try:
        for idx, contract in enumerate(contracts, 1):
            ...
            if saved > 0:
                ...
                print(f"    [SAVE] {query_key} → {query_key}: 保存 {saved} 条K线数据")
        ...
        print(f"[ERROR] K线同步失败: {e}")
        log_error(FSM, f"K线同步失败: {e}", notify=True)
        traceback.print_exc()
        return {
