...
| `permission_views.py` | IAM permission views — PermissionRequest, DirectPermission, Business, Project, etc. | `PermissionRequestViewSet` — <br>`UserDirectPermissionViewSet` — <br>`BusinessGroupViewSet` — BusinessGroup CRUD — superuser only<br>`BusinessViewSet` — Business CRUD + member management<br>`DeployEnvironmentViewSet` — DeployEnvironment CRUD + permission management — superuser for write<br>`search_users()` — 搜索用户，返回 async_select 格式 {data: [{value: id, label: username}]}<br>`my_permissions()` — Return current user's unified IAM permissions for a given project.<br>`my_full_permissions()` — 返回当前用户的全部权限 codename 列表（含平台级和项目级）<br>`page_permissions()` — Return visible tabs and buttons for a given app based on user permissions.<br>`permission_catalog()` — 返回所有子产品的权限目录 — 按 app → tab → button 组织，用于权限申请页面 |
| `role.py` | @author: 猿小天
@contact: QQ:1638245306
...
| `user.py` | — | `UserSerializer` — 用户管理-序列化器<br>`UserCreateSerializer` — 用户新增-序列化器<br>`UserUpdateSerializer` — 用户修改-序列化器<br>`UserInfoUpdateSerializer` — 用户修改-序列化器<br>`ExportUserProfileSerializer` — 用户导出 序列化器<br>`recursion()` —  |
