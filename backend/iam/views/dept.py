...
from iam.models import IAMDept, IAMUsers
...
class DeptSerializer(CustomModelSerializer):
    ...
    def get_dept_user_count(self, obj: IAMDept):
        return IAMUsers.objects.filter(dept=obj).count()

    def get_hasChild(self, instance):
        hasChild = IAMDept.objects.filter(parent=instance.id)
        if hasChild:
            return True
        return False

    def get_status_label(self, obj: IAMDept):
        if obj.status:
            return "启用"
        return "禁用"

    def get_has_children(self, obj: IAMDept):
        return IAMDept.objects.filter(parent_id=obj.id).count()
...
class DeptCreateUpdateSerializer(CustomModelSerializer):
    ...
    def create(self, validated_data):
        ...
        dept_obj = IAMDept.objects.filter(parent=self.request.user.dept).order_by('-sort').first()
...
class DeptViewSet(CustomModelViewSet, ImportSerializerMixin):
    ...
    def dept_lazy_tree(self, request, *args, **kwargs):
        ...
        if is_superuser:
            queryset = IAMDept.objects.values('id', 'name', 'parent')
        else:
            queryset = IAMDept.objects.values('id', 'name', 'parent')
    ...
    def move_up(self, request):
        ...
        try:
            dept = IAMDept.objects.get(id=dept_id)
        except IAMDept.DoesNotExist:
            return ErrorResponse(msg="部门不存在")
        previous_menu = IAMDept.objects.filter(sort__lt=dept.sort, parent=dept.parent).order_by('-sort').first()
        if previous_menu:
    ...
    def move_down(self, request):
        ...
        try:
            ...
            return ErrorResponse(msg="部门不存在")
        next_menu = IAMDept.objects.filter(sort__gt=dept.sort, parent=dept.parent).order_by('sort').first()
        if next_menu:
    ...
    def dept_info(self, request):
        """部门信息"""
        def inner(did, li):
            sub = IAMDept.objects.filter(parent_id=did)
            if not sub.exists():
                return li
            for i in sub:
                li.append(i.pk)
        ...
        if int(show_all):  # 递归当前部门下的所有部门，查询用户
            ...
            users = IAMUsers.objects.filter(dept_id__in=all_did)
        else:
            if dept_id != '':
                users = IAMUsers.objects.filter(dept_id=dept_id)
            else:
                users = IAMUsers.objects.none()
        dept_obj = IAMDept.objects.get(id=dept_id) if dept_id != '' else None
        sub_dept = IAMDept.objects.filter(parent_id=dept_obj.pk) if dept_id != '' else []
        data = {
        ...
        for dept in sub_dept:
            ...
            sub_data = {
                'name': dept.name,
                'count': IAMUsers.objects.filter(dept_id__in=all_did).count()
            }
