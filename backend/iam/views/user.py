...
from iam.models import IAMUsers, IAMDept
from common.utils.json_response import ErrorResponse, DetailResponse, SuccessResponse
...
class UserViewSet(CustomModelViewSet):
    ...
    queryset = IAMUsers.objects.exclude(is_superuser=1).all()
    ...
    import_field_dict = {
        ...
        "dept": {"title": "部门", "choices": {"queryset": IAMDept.objects.filter(status=True), "values_name": "name"}},
    ...
    def reset_to_default_password(self, request, *args, **kwargs):
        """恢复默认密码"""
        instance = IAMUsers.objects.filter(id=kwargs.get("pk")).first()
        if instance:
    ...
    def reset_password(self, request, pk):
        ...
        instance = IAMUsers.objects.filter(id=pk).first()
        data = request.data
        new_pwd = data.get("newPassword")
        new_pwd2 = data.get("newPassword2")
        if instance:
            ...
            if dept_id != '':
                ...
                if searchs:
                    ...
                    for i in searchs[1:]:
                        q |= i
                    q_obj.append(Q(q))
                queryset = IAMUsers.objects.filter(*q_obj, dept_id__in=all_did)
            else:
