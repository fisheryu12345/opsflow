...
> 上次自动更新: 2026-06-28 | 触发提交: 2c58d8d3
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | (无文档说明) | (无公开组件) |
| `apps.py` | (无文档说明) | `OpsflowConfig` —  |
| `serializers.py` | (无文档说明) | `GlobalVariableField` — 全局变量字段 — 接受扁平或结构化格式，始终返回结构化格式<br>`FlowTemplateSerializer` — <br>`TemplateVersionSerializer` — <br>`FlowExecutionSerializer` — <br>`TemplateNodeSerializer` —  |
| `tasks.py` | (无文档说明) | `run_async()` — 在 Celery worker 中安全执行异步协程。<br>`execute_pipeline_task()` — Celery 任务 — 异步执行 Pipeline<br>`auto_retry_node_task()` — Celery 任务 — 自动重试失败节点（由 auto_retry dispatch 触发）<br>`execute_node_timeout_strategy()` — Celery 任务 — 执行节点超时策略<br>`webhook_send()` — Celery 任务 — 发送 Webhook 回调 |
| `urls.py` | (无文档说明) | (无公开组件) |
...
| `scheduler_service.py` | (无文档说明) | `OpsflowScheduler` — APScheduler wrapper for managing SchedulePlan timed triggers |
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | Sugiyama layered graph layout engine for OPSflow. | (无公开组件) |
...
| `elements.py` | pipeline 构建 - 节点元素创建 | (无公开组件) |
| `validation.py` | pipeline 构建 - 循环引用检测 | (无公开组件) |

## `core/variable_types/`

| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | 变量类型包 — 导入此模块触发所有变量类注册到 VARIABLE_REGISTRY | (无公开组件) |
...
| `seed_opsflow.py` | Seed OpsFlow reference & mock data | `Command` — <br>`Command` —  |
| `seed_template_presets.py` | Seed Template Presets — 10 个常见 IT 运维场景预设提示词 (中英双语) | `Command` —  |
| `start_opsflow_scheduler.py` | (无文档说明) | `Command` —  |
...
|------|------|----------|
| `__init__.py` | (无文档说明) | (无公开组件) |
| `audit.py` | (无文档说明) | `OperationRecord` — 操作审计记录 — 记录所有重要用户操作<br>`OpsLog` — 审计日志 — 每一步的详细执行记录 |
| `auth.py` | (无文档说明) | `ApiToken` — 外部 API Token — 用于第三方系统认证，绑定 Business + Deployment Environment |
| `env.py` | (无文档说明) | `ProjectEnvironmentVariable` — 项目级环境变量 — 跨模板共享的配置值 |
| `execution.py` | (无文档说明) | `FlowExecution` — 执行实例 — 一次流程运行记录<br>`ExecutionNode` — 执行节点 — 执行实例中的节点记录，从 TemplateNode 同步<br>`ExecutionScheme` — 执行方案 — 预定义的节点排除集 + 变量覆盖<br>`AutoRetryStrategy` — 节点自动重试策略 — 在信号拦截到 FAILED 时自动触发重试<br>`NodeTimeoutConfig` — 节点超时配置 — 绑定到执行实例的节点超时策略 |
| `knowledge.py` | (无文档说明) | `OpsKnowledge` — RAG 知识库 — 历史案例/故障/文档 |
| `plugin.py` | (无文档说明) | `PluginMeta` — 标准插件元数据 — 注册时自动同步（支持多版本） |
| `schedule.py` | (无文档说明) | `SchedulePlan` — 调度计划 — 一次性或周期性自动执行 |
| `template.py` | (无文档说明) | `FlowTemplate` — 编排模板 — AI 生成的或人工创建的流程定义<br>`TemplateVersion` — 模板版本历史 — 每次发布时创建<br>`TemplateNode` — 模板节点 — 从 pipeline_tree JSON 同步为独立行，支持 SQL 查询<br>`TemplateCollect` — 用户收藏的模板<br>`TemplateCategory` — 模板分类 — 可管理配置，管理员可增删 |
| `webhook.py` | (无文档说明) | `WebhookConfig` — Webhook 回调配置 — 绑定到模板，执行完成后触发<br>`WebhookLog` — Webhook 投递日志 |
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | Agent 远程执行插件 — 通过 Agent 在被管控主机上执行命令和脚本 | (无公开组件) |
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | ITSM plugin package — create/update tickets from OpsFlow executions | (无公开组件) |
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | Pipeline 信号处理器 — 追踪 BambooDjangoRuntime ERI 状态变更 | (无公开组件) |
...
| `helpers.py` | Helper functions — 信号处理中使用的工具函数 | (无公开组件) |
| `state.py` | State management — 节点状态持久化和状态树增量更新 | (无公开组件) |
| `timeout.py` | 超时追踪 — 节点状态变更时更新 Redis 超时集合 | (无公开组件) |
| `trace.py` | Trace management — 节点执行轨迹、日志记录、OpsLog 写入 | (无公开组件) |
...
| `knowledge_views.py` | (无文档说明) | `OpsKnowledgeViewSet` —  |
| `log_views.py` | (无文档说明) | `OpsLogViewSet` — OpsLog 通过 execution.project 间接隔离，不直接设 project FK |
...
| `schedule_views.py` | (无文档说明) | `SchedulePlanViewSet` —  |
...
| 文件 | 用途 | 核心组件 |
|------|------|----------|
| `__init__.py` | Dashboard API endpoints — 为 urls.py 向后兼容重新导出 | (无公开组件) |
