...
| `auto_retry.py` | 自动重试策略 — 节点 FAILED 时自动触发重试 | dispatch_auto_retry() — 检查并派发自动重试 — 在信号拦截到 FAILED 时调用, AutoRetryStrategyCreator — 批量创建自动重试策略 — 在 FlowEngine.run() 开始时调用 |
...
| `error_codes.py` | 标准化错误码 — 全系统统一错误分类和封装 | api_success() — 标准成功响应, api_error() — 标准错误响应, ErrorCodes — 错误码常量 |
...
| `llm_service.py` | LLM 服务 — 通过集成中心调用 AI 完成流程生成/分析 | generate_pipeline() — Convert natural language to Pipeline Tree JSON, refine_pipeline() — Multi-turn: modify existing Pipeline Tree based on new instruction, rag_search() — Simple text RAG search — recall relevant cases from OpsKnowledge |
| `mako_resolver.py` | Mako 模板变量解析 — 安全沙箱封装 | resolve_with_mako() — 通用入口 — 检测 #{...} 走 Mako 路径，否则原样返回, MakoResolver — 安全的类 Mako 表达式解析器 |
| `node_dispatcher.py` | NodeCommandDispatcher — 节点操作调度器 | NodeCommandDispatcher — 节点操作调度器 |
| `node_sync.py` | 节点同步工具 — 在 pipeline_tree JSON 与 TemplateNode/ExecutionNode 模型间同步 | extract_nodes_from_tree() — 从 pipeline_tree JSON 提取节点列表（标准格式，非可视化节点）, sync_template_nodes() — 将模板的 pipeline_tree 同步为 TemplateNode 行（全量删除重建）, sync_execution_nodes() — 从模板快照创建 ExecutionNode 记录 |
| `node_timeout_strategy.py` | 节点超时策略 — 超时后自动执行 forced_fail / forced_fail_and_skip | batch_create_timeout_configs() — 遍历 pipeline_tree，为 timeout_seconds > 0 的节点创建超时配置, get_redis() — 获取 Redis 连接, update_node_timeout() — 更新节点超时状态 — 在信号处理器中调用, NodeTimeoutStrategy — 节点超时策略基类, ForcedFailStrategy — 强制失败策略 — 调用 api.forced_fail_activity, ForcedFailAndSkipStrategy — 强制失败并跳过策略 |
| `pipeline_preview.py` | Pipeline 预览与清理 — 执行方案预览树生成 | PipelinePreviewService — 执行方案预览服务 — 根据排除节点生成清理后的 pipeline 树 |
| `pipeline_schema.py` | Pipeline Tree JSON Schema 验证 | validate_pipeline_schema() — JSON Schema 结构验证, validate_global_var_keys() — 校验全局变量键名格式, validate_pipeline_full() — 全量验证：JSON Schema → 键名 → 返回 unified 结果 |
...
| `safety_guard.py` | Pipeline Tree 安全校验器 — 原子白名单从 PLUGIN_REGISTRY 动态加载 | WHITELIST_ATOMS() — 返回所有已注册插件的 code 集合, HIGH_RISK_ATOMS() — 返回 risk_level == 'high' 的插件 code 集合, BACKUP_REQUIRED_ATOMS() — 已废弃 — BasePlugin 不再追踪 dependencies，始终返回空集 |
| `scheduler_service.py` | — | OpsflowScheduler — APScheduler wrapper for managing SchedulePlan timed triggers |
| `states.py` | 状态枚举与流转矩阵 — 节点生命周期 + Pipeline 级别状态 | validate_node_transition() — 检查节点状态转移是否合法，返回 True/False, validate_pipeline_transition() — 检查 Pipeline 级状态转移是否合法, map_bamboo_node_state() — bamboo-engine 状态 → NodeState, NodeState — 节点状态枚举 — 每个节点的生命周期状态, PipelineState — Pipeline 级状态枚举 — FlowExecution.status |
...
| `variable_resolver.py` | 变量解析引擎 — ${key} 模板变量替换 + 跨节点数据引用 | normalize_global_vars() — 规范化 global_vars 为包含元数据的结构化格式, get_global_vars_values() — 从规范化（或扁平）global_vars 中提取纯值 dict, count_variable_references() — 扫描 pipeline_tree 中 `${var_key}` 的出现次数 |
...
| `acyclic.py` | Acyclic graph transformation — remove self-loops and reverse backward edges. | remove_self_edges() — Remove self-looping edges and store them for restoration., insert_self_edges() — Restore previously removed self-loop edges., acyclic_run() — Reverse backward edges until graph is acyclic. Returns reversed flows. |
...
| `dummy.py` | Long-edge splitting with dummy nodes + gateway fill-number computation. | replace_long_path_with_dummy() — Replace edges spanning >1 rank with chains of DummyNode -> DummyFlow., compute_nodes_fill_num() — Calculate extra vertical slots needed at each position for gateway branches., remove_dummy() — Remove dummy nodes/flows and restore long edges. |
| `layout_adapter.py` | OPSflow {nodes, edges} <-> pipeline tree format bridge. | opsflow_to_pipeline() — Convert OPSflow {nodes, edges} to a pipeline tree dict., pipeline_to_positions() — Extract [{id, x, y}] from pipeline after draw_pipeline()., compute_layout() — Full layout computation: OPSflow format in -> positions out. |
| `normalize.py` | Pipeline data normalization — build/remove all_nodes dictionary. | get_all_nodes() — Build flat all_nodes dict from pipeline sections., normalize_run() — , normalize_undo() —  |
...
| `tests.py` | Unit tests for the Sugiyama layout engine. | test_empty_graph() — , test_single_node() — , test_two_nodes() —  |
| `utils.py` | Utility functions for flow ID management on nodes. | format_to_list() — Normalize incoming/outgoing to list., add_flow_id_to_node_io() — Add flow_id to node's incoming or outgoing list., delete_flow_id_from_node_io() — Remove flow_id from node's incoming or outgoing list. |
...
| `order.py` | Crossing minimization via weighted-median heuristic (Sugiyama). | ordering() — Assign node order within each rank to minimize edge crossings., init_order() — Initialize per-rank node ordering via topological traversal from start., wmedian() — Apply weighted-median heuristic in alternating direction. |
...
| `utils.py` | Rank utility functions. | max_rank() — , min_rank() — , normalize_ranks() — Shift all ranks so that min rank is 0. |
...
| `clean_expired_locks.py` | Clean expired template editing locks — heartbeat > 60s old. | Command —  |
| `clean_node_trace_logs.py` | 清理 N 天前的节点轨迹日志文件 | Command —  |
| `clean_opsflow_data.py` | 清理过期 OpsFlow 数据 — DB 记录 + 日志文件 | Command —  |
| `fix_node_status_double_encode.py` | Management command — 修复 MySQL JSON_SET 双层编码导致的 node_status 值错误 | needs_fix() — 检查值是否带双层引号：字符串以 " 开头和结尾且长度 > 2, fix_value() — 去掉外层引号：'"completed"' → 'completed', Command —  |
...
| `seed_opsflow.py` | Seed OpsFlow reference & mock data | Command — , Command —  |
...
| `auth.py` | — | ApiToken — 外部 API Token — 用于第三方系统认证，绑定 Business + Deployment Environment |
...
| `registry.py` | 插件注册中心 — 自动扫描 plugins/ 下所有 BasePlugin 子类 | discover_plugins() — 扫描 opsflow/plugins/ 下所有模块，自动注册 BasePlugin 子类, get_plugin() — 获取插件类，version=None 时返回最新版本, get_plugin_versions() — 返回指定 code 的所有版本列表 |
...
| File | Purpose / 用途 | Core Components / 核心组件 |
|------|----------------|---------------------------|
| `__init__.py` | Ansible Tower (AWX) REST API 服务封装 — 插件后端执行引擎 | get_tower_service() — 获取 TowerService 单例, TowerService — Ansible Tower (AWX) REST API 服务 |
...
|------|----------------|---------------------------|
| `__init__.py` | — | — |
| `dr_service.py` | DR 拓扑服务 — Neo4j 查询 + AI Prompt + 拓扑描述 | get_dr_group_topology() — 基于 Neo4j 查询 DR 组拓扑—Application 级, neighbors_to_pipeline() — 将 Application 级拓扑数据生成为标准 pipeline 格式（支持 CALLS 依赖排序）, build_topology_description() — 将拓扑数据转为 AI 可读的文本描述（Application 级） |
...
| `test_execution_api.py` | API integration tests — FlowExecutionViewSet | ExecAPITestBase — , ExecutionListTest — , ExecutionDetailTest —  |
| `test_flow_engine.py` | FlowEngine 测试 — 状态管理 + run/retry/skip | TestFlowEngineStateManagement — FlowEngine 状态管理 — 不需要 bamboo 依赖, TestFlowEngineRun — FlowEngine.run() — pipline 构建+执行, TestFlowEngineWSNotification — WebSocket 通知（best-effort） |
...
| `test_manual_pause.py` | Manual Pause 原子测试 — ManualPausePlugin / PluginService 集成 | TestManualPausePlugin — ManualPausePlugin 原子, TestPluginServiceManualPause — PluginService.execute 中 manual_pause 暂停触发 |
...
| `test_optional_skip.py` | Optional 节点失败自动跳过测试 — _check_optional_skip / on_post_set_state 集成 | TestCheckOptionalSkip — _check_optional_skip 辅助函数, TestOptionalSkipInSignal — on_post_set_state 信号中 optional skip 集成 |
...
| `test_template_api.py` | API integration tests — FlowTemplate edit lock | TemplateLockTest — TemplateLock acquire/release actions (do not use ProjectFilteredViewset) |
| `test_template_lock.py` | TemplateLock tests — acquire/release/heartbeat/expiry/update guard | TemplateLockAcquireTest — acquire_lock happy path and conflict detection, TemplateLockExpiryTest — is_expired and heartbeat timeout, TemplateLockReleaseTest — release_lock idempotency |
...
|------|----------------|---------------------------|
| `__init__.py` | — | — |
| `aliyun_views.py` | 阿里云 ECS 资源查询 API — 供表单 async_select 动态加载数据 | describe_images() — , describe_instance_types() — , describe_security_groups() —  |
| `audit_views.py` | 操作审计视图 — 只读查询接口 | OperationRecordViewSet — 操作审计记录 — 只读 |
| `base.py` | ViewSet base class — project isolation + multi-tenant permission support | ProjectFilteredViewSet — Project-scoped ViewSet with multi-tenant permission enforcement., ProjectReadOnlyViewSet — Read-only variant — disables all write operations. |
...
| `analytics.py` | Dashboard — 分析/分布端点（top_templates, user_activity, 分布, 排行等） | dashboard_top_templates() — Top N templates ranked by execution count (with success rate and average duratio, dashboard_user_activity() — User execution activity statistics, dashboard_status_distribution() — Execution status distribution |
