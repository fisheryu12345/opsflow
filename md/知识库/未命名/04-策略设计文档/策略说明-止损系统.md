...
└─────────────────────────────────────────────────────────────────────┘
   │
   ├── 订阅实时行情：quote.last_price vs 止损价
...
def check_and_execute_stop_loss(api):
    # 用 TqSDK 实时行情判断触发止损（不使用 DB 最新收盘价）
    for position in 所有有持仓:
    ...
    for position in positions:
        ...
        # 创建 STOP_LOSS 信号并标记 EXECUTING
        # TargetPosTask 平仓（timeout=60s）
...
| 每日收盘入口 | `job_daily_close_calculation()` | `scheduler/tasks_daily_close.py:545` |
| 更新最高/最低价 | `update_all_positions_high_low_price()` | `scheduler/tasks_daily_close.py:344` |
| 更新止损价 | `update_all_positions_stop_loss_price()` | `scheduler/tasks_daily_close.py:388` |
| 多头止损公式 | `highest_close - 2*(1+factor)*ATR` | `scheduler/tasks_daily_close.py:448` |
| 空头止损公式 | `lowest_close + 2*(1+factor)*ATR` | `scheduler/tasks_daily_close.py:471` |
| 保本启用检查 | `盈利 > PROTECT_COST_ENABLED_RATIO * ATR` | `scheduler/tasks_daily_close.py:492/500` |
| 保本价计算 | `cost_price ± tick * 2` | `scheduler/tasks_daily_close.py:483/485` |
| 保本兜底 | `dynamic_stop_loss vs protect_price` | `scheduler/tasks_daily_close.py:510-511/517-518` |
| 止损触发检查（收盘后） | `check_exit_signals()` | `scheduler/tasks_daily_close.py:87` |
| 止损执行（临收盘） | `check_and_execute_stop_loss()` | `infrastructure/stop_loss_executor.py:68` |
| 信号防重复 | `check_duplicate_pending_signal()` | `core/signal_checker.py:8` |
| 趋势因子配置 | `TREND_GAP_LIMIT` | `core/config_loader.py:15` |
| 保本配置 | `PROTECT_COST_ENABLED_RATIO` | `core/config_loader.py:13` |
