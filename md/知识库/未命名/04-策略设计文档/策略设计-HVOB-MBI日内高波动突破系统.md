...
> 涉及文件: `hvob_mbi/config.py`, `hvob_mbi/screening.py`, `hvob_mbi/mbi.py`, `hvob_mbi/trading_engine.py`, `hvob_mbi/signal_recorder.py`, `hvob_mbi/models.py`, `hvob_mbi/tasks.py`, `stock/models.py` (FullContractList.night_trading), `hvob_mbi/management/commands/hvob_trading.py`, `stock/infrastructure/order_execution.py`（两步开仓）
...
| `LAST_ENTRY_TIME` | 14:30 | 最后入场时间（仅为配置定义，引擎使用 FORCE_CLOSE_TIME 判定结束） |
...
- **5 分钟 K 线：** `api.get_kline_serial(symbol, 300, 50)` — `_subscribe_all()` 阶段订阅，当前用于维持行情连接活跃度（MBI 计算实际基于 Quote 数据，非 K 线）
...
HVOB 引擎在多种场景下需要恢复状态：程序崩溃重启、20:55/08:55/13:25 定时重启、白天迟到启动。恢复策略因相位不同而异。

#### 7.10.1 状态持久化节点

| 触发时机 | 方法 | 持久化内容 |
|---------|------|-----------|
| 盘前筛选完成 | `_save_watchlist_items()` | 写入 `HvobMbiWatchlistItem`（独立表，`bulk_create` + `ignore_conflicts=True`） |
| 夜盘 OR 关闭 (21:30) | `_save_night_or_state()` | 写入 `HvobMbiDailyState.night_opening_ranges` |
| 日中转换 (9:30, gap_check→day_breakout) | `_save_midday_state()` | 写入 `HvobMbiDailyState` 全部字段（OR/MBI/banned/traded/计数器），`is_complete=False` |
| 收盘 (14:55+) | `_save_daily_state()` | 覆盖更新 `HvobMbiDailyState`，`is_complete=True` |

#### 7.10.2 恢复路径

**night_breakout 阶段启动（21:30+ 启动或重启）：**
```
_restore_night_breakout_state()
  ├─ 尝试从 HvobMbiDailyState.night_opening_ranges 恢复夜盘 OR
  ├─ 无已保存数据，且当前时间 ≥ 21:30 → 直接调用 _close_night_opening_range()
  │  用 K 线计算（数据来自 TqSDK 服务器，崩溃安全）
  └─ 0:00-9:00 凌晨启动 → 有前一晚已保存的 OR，走正常恢复路径
```

**day_breakout 阶段启动（9:30+ 启动或重启）：**
```
_restore_day_breakout_state()
  ├─ 从 HvobMbiDailyState 读取 opening_ranges / mbi_value / mbi_label
  ├─ 恢复 banned_symbols → self.banned
  ├─ 恢复 traded_symbols → self.traded
  ├─ 从 DailyStrategySignal 补全 traded（_supplement_traded_from_db）
  │  （捕获保存在 traded 之后、崩溃前已完成但未保存到状态的成交）
  ├─ 从 PositionState 重建 self.positions（_restore_positions）
  │  （使用 h20_price / l20_price 恢复 OR_R，仅限 R>0 的持仓）
  └─ 恢复 total_trades / daily_pnl 计数器
```

**定时重启（非相位切换）：**
```
_check_restart → _reconnect
  ├─ safe_close_api() → sleep(35s) → create_tqapi() → wait_update(15s)
  ├─ _subscribe_all() 重新订阅行情（新 api 实例需重新 get_quote/get_kline_serial）
  ├─ wait_update(10s)
  └─ 所有内存状态（positions / opening_ranges / banned / traded / watchlist）完整保留
```
定时重启不改变相位，`_last_restart_key`（`%Y-%m-%d-%H%M`）防同一分钟重复触发。

#### 7.10.3 关键设计原则

1. **K 线法计算 OR（崩溃安全）**：夜盘 21:30 和日盘 9:30 均优先使用 `get_kline_serial(symbol, 60, 30)` 获取最近 30 根 1 分钟 K 线计算 H/L/R。数据来自 TqSDK 服务器而非本地内存，程序崩溃重启后仍可正确计算完整区间。
2. **筛选只执行一次**：`_do_screening` 仅在启动时执行一次，定时重启通过 `_reconnect` 保留内存 `watchlist`，不重新筛选。避免观察池变化导致已持仓品种丢失。
3. **Day Breakout 恢复失败即跳过**：`_restore_day_breakout_state()` 返回 False 时引擎直接退出（`phase='done'`），不尝试无数据交易。
4. **三路 traded 恢复**：`traded_symbols` 来自日中状态 + `DailyStrategySignal` 交叉验证 + `PositionState` 持仓重建，确保已交易品种不会二次开仓。
...
| opening_ranges | JSONField | 日盘开盘区间 `{symbol: {H,L,R}}` |
| night_opening_ranges | JSONField | 夜盘开盘区间 `{symbol: {H,L,R,closed}}`（崩溃恢复用） |
| banned_symbols | JSONField | 跳空穿越拉黑品种 |
| traded_symbols | JSONField | 已交易品种 |
| total_trades | IntegerField | 交易次数 |
| daily_pnl | DecimalField | 日盈亏 |
| daily_loss | DecimalField | 日累计亏损（日内风控预留） |
...
**调度器启动（APScheduler）：**
`tasks.py` 中的 `hvob_mbi_trading_task()` 函数通过 APScheduler 定时触发，支持多账户并行。遍历所有 `is_active=True` 的 `HvobMbiConfig`，依次为每个账户创建 TqSDK API 实例并启动 `HvobTradingEngine`。

**运行方式：** TqSDK `wait_update` 事件循环，从启动持续运行到 14:55 强制平仓后自动退出。建议通过 systemd / supervisor 管理进程生命周期，或通过 APScheduler 定时触发。
...
### 10.10 HIGH-29：两步开仓 volume_short 记录错误持仓量（signal_recorder 平仓参数 BUG）

HVOB 止损信号写入失败，导致 lu2607/sc2607 两条止损信号状态 `PENDING`、PositionState 未被重置、ClosedPositionRecord 缺失。

**根因 1（signal_recorder）：** `_create_closed_record()` 接受 9 个位置参数 `(account, symbol, product_code, trade_date, direction, exit_price, pnl, cost_price, volume)`，但 `record_stop_loss_signal` 多传了一个 `'STOP_LOSS'` 字符串作为第 10 个参数：
```python
# BUG: 10个参数，_create_closed_record 只接受9个
_create_closed_record(account, symbol, product_code, trade_date,
                      direction, exit_price, pnl, cost_price, volume, 'STOP_LOSS')
```
导致 `TypeError: _create_closed_record() takes 9 positional arguments but 10 were given`。`DailyStrategySignal.create()` 先于 `_create_closed_record` 执行已成功，但平仓记录和持仓重置均未执行。

**根因 2（两步开仓记录）：** `support_open_min_volume=True` 时 TqSDK 可能用反向开仓代替平仓来满足最小开仓限制，导致 `volume_long`/`volume_short` 反映的是总开仓量而非净持仓（例：开 4 手空 + 1 手多 → `volume_short=4` 但净持仓 `pos.pos=3`）。`execute_two_step_opening` 原使用 `volume_short` 记录实际持仓量导致记录数偏大。

**修复要点：**
- `signal_recorder.py`：移除 `record_stop_loss_signal` 中多余的 `'STOP_LOSS'` 参数
- `order_execution.py`：两步开仓使用 `abs(pos_after.pos)` 代替 `volume_long`/`volume_short`

**数据库修复（2026-05-27）：**
- 补充写入 `INE.lu2607`（5手@4676, PnL=1700）和 `INE.sc2607`（1手@593.5, PnL=-100）的 `ClosedPositionRecord`
- 重置 `PositionState` 为 cleared（units=0, direction=0）
- 更新 ID=179（lu2607）和 ID=180（sc2607）的 `DailyStrategySignal` 状态从 `PENDING` 为 `SUCCESS`
