...
## ✅ LOW-14: HVOB 观察池默认加载全部日期数据 — 已修复 (2026-05-22)

**文件**: [hvob_mbi/views.py](../../backend/hvob_mbi/views.py#L31-L38)

**问题描述**:
HVOB 观察池页面打开时，未传 `trade_date` 参数，后端返回所有历史日期的记录（按 `-trade_date` 排序），用户需要手动选择日期筛选，体验不佳。

**影响分析**:
- 用户每次打开页面需要手动选择日期才能看到当天观察池
- 页面默认展示多条数据，信息过载

**修复内容**:
在 `HvobMbiWatchlistViewSet` 中重写 `get_queryset()`，未传 `trade_date` 参数时自动聚合到最新日期：
