...
## ✅ MEDIUM-26 (TqSDK): 入场/平仓价格使用 quote.last_price 而非实际成交价 — 已修复 (2026-05-19)
...
- [infrastructure/stop_loss_executor.py:64-97](../backend/stock/infrastructure/stop_loss_executor.py#L64)（止损平仓 — 已修复）
...
**修复内容**:
1. **Exit 平仓路径** (order_signals.py:522-539): 改为优先 `api.get_trade()` 遍历成交记录计算实际均价，无成交时回退 `quote.last_price`
2. **止损平仓路径** (stop_loss_executor.py:64-97): 已正确使用 `api.get_trade()` 计算实际均价

**Add-on 加仓与 Rollover 换月路径不修正的原因**:
- 这两个路径仍使用 `quote.last_price` 近似价格作为成本
- 但系统有**定期成本价同步机制**（每日 `update_all_positions_stop_loss_price` 从 TqSDK 的 `open_price_long`/`open_price_short` 同步），偏差会在当日收盘前自动修正
- 加仓与换月的成本价偏差存在时间窗口短（最迟当天 15:32 收盘任务中修正），对止损计算的实际影响有限
- 因此不额外修改，依赖现有同步机制兜底
...
## ✅ MEDIUM-35: 止损平仓循环误用 target_pos.is_finished() + 超时后资源泄漏 — 已修复 (2026-05-19)

**文件**: [stop_loss_executor.py](../backend/stock/infrastructure/stop_loss_executor.py)

**问题描述**:
止损平仓等待循环使用了 `while not target_pos.is_finished()` 作为条件，存在两个问题：

1. **循环条件错误**：`target_pos.is_finished()` 检查的是 TargetPosTask **自身是否已完成发单**，而非持仓是否真的归零。发单完成与实际成交之间有时间差，用此条件退出循环过早。
2. **超时后不释放资源**：超时直接 `return False`，未调用 `target_pos.cancel()`，导致 TargetPosTask 持有的订单和工作协程泄漏。
3. **deadline 不衰减**：`deadline=time.time() + 1` 固定 1s，超时后仍可能多等一整秒。

**影响分析**：
- 止损平仓可能在持仓未完全归零时就认为"完成"
- 超时后的资源泄漏积累可能影响 TqApi 连接的稳定性

**修复内容**：
1. 循环条件改为 `while time.time() - start_time < TIMEOUT_SECONDS`（超时驱动）
