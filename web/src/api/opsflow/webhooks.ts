...
export function GetWebhooks(templateId: number) {
  return opsflowRequest({ url: prefix + `templates/${templateId}/webhooks/`, method: 'get' })
}
...
export function CreateWebhook(templateId: number, data: {
  ...
  return opsflowRequest({ url: prefix + `templates/${templateId}/webhooks/`, method: 'post', data })
}
